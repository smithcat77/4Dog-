...
class Controller:
    ...
    def __init__(self, ttyStr='/dev/ttyACM0', device=0x0c, retry_interval=1, max_retries=None):
        self.ttyStr = ttyStr
        self.device = device
        ...
        self.Targets = [0] * 24  # Track target position for each servo
        self.Mins = [0] * 24     # Servo minimum targets
        self.Maxs = [0] * 24     # Servo maximum targets
    ...
    def connect(self):
        ...
        while True:
            try:
                self.usb = serial.Serial(self.ttyStr, timeout=1)  # Attempt to open the serial port
                print("Connected successfully to", self.ttyStr)
                break  # Exit the loop if connection is successful
            except serial.SerialException as e:
                print(f"Failed to connect to {self.ttyStr}: {e}. Retrying in {self.retry_interval} seconds...")
